...
flag = Path().home().joinpath("Appdata/Local/Temp/CSHRn")
...
if __name__ == "__main__":
    ...
    if ((sys.argv[1:].__contains__("-nd") or sys.argv[1:].__contains__("--nodebug"))) or not DEV:
        DEBUG: bool = False
    ...
    ART_PATH: Path = Path("_internal/asciiart/")
    ...
    {"\n ".join([f"[blue bold]{stat}[/blue bold]: {val}" for stat, val in displayStats.items()])}"""
        ...
        cli.print(HorizontalLayout(f"{kwargs["-asciiart"]}-{kwargs['-asciiversion']}", "left").combineText(tst, "blue bold"))
